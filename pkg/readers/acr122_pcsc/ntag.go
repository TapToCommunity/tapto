...
func readBlock(tag *scard.Card, block byte) ([]byte, error) {
	return tag.Transmit([]byte{0xFF, 0xB0, 0x00, block, 0x04})
}

func ReadNtag(tag *scard.Card) (TagData, error) {
	blockCount, err := getNtagBlockCount(tag)
	if err != nil {
		return TagData{}, err
	}
	...
	if bytes.Equal(header[9:], AMIIBO_MATCHER) {
		...
		amiibo, err := readBlock(tag, byte(21))
		if err != nil {
			return TagData{}, err
	...
	for i := 0; i <= (blockCount / 4); i++ {
		blocks, err := readBlock(tag, byte(currentBlock))
		if err != nil {
			return TagData{}, err
		}
...
func getNtagBlockCount(tag *scard.Card) (int, error) {
	// Find tag capacity by looking in block 3 (capability container)
	res, err := readBlock(tag, 0x00)
	if err != nil {
		return 0, err
	} else if len(res) < 1 {
