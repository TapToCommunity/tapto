...
func Exists(platform platforms.Platform) bool {
	_, err := os.Stat(filepath.Join(platform.ConfigFolder(), config.GamesDbFile))
	return err == nil
}
...
func open(platform platforms.Platform, options *bolt.Options) (*bolt.DB, error) {
	err := os.MkdirAll(filepath.Dir(filepath.Join(platform.ConfigFolder(), config.GamesDbFile)), 0755)
	if err != nil {
		return nil, err
	}

	db, err := bolt.Open(filepath.Join(platform.ConfigFolder(), config.GamesDbFile), 0600, options)
	if err != nil {
		return nil, err
	}
